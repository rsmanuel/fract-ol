...
int input_validation(char **str, int argc)
{
    ...
    if (str[1][0] == 'M')
    {
        if (!strcmp(str[2], "blue") || !strcmp(str[2], "red") || !strcmp(str[2], "green") || !strcmp(str[2], "white"))
            return (1);
        return(-1);
    }
    else if (str[1][0] == 'J')
    {
        if (str[2][0] != '1' && str[2][0] != '2')
            return (-1);
        if (!strcmp(str[3], "blue") || !strcmp(str[3], "red") || !strcmp(str[3], "green") || !strcmp(str[3], "white"))
            return (1);
        else
            return (-1);
    }
...
void validation_fail(void)
{
	...
	ft_putcolor_fd(ANSI_COLOR_GREEN, "./fractol <fractal> <option (only julia set)> <color>\n\n", 1);
